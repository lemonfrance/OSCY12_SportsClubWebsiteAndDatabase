...
$data = "Upcoming Event";
	$tablename = "club_bulletin";
	$addel = "added to";
	
	if (isset($_POST['upc_ttl'])){
		echo "Successfully added upcoming event title. <br>";
		$upc_ttl = $_POST['upc_ttl'];
	}
	else{
		echo "No title added; left null. <br>";
		$upc_ttl = "";
	}
	
	if (isset($_POST['upc_body'])){
		echo "Successfully added upcoming event content. <br>";
		$upc_body = $_POST['upc_body'];
	}
	else{
		echo "No content added; can't be left null. Failed adding new upcoming event. <br>";
	}
	
	$sql = "INSERT INTO $tablename(BulletinSection, Title, Body) VALUES('Upcoming Events','$upc_ttl','$upc_body')";
	include "koac_webexec16.php";
