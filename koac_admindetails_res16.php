...
<title> Database (Details) - Admin </title>
...
body{
 ...
 <p class='titles' id='bodyttl'> Database - Details </p>

 <p>
 <font id='orangefont'>
 <form method='POST'>
 <input type='text' id="searchbox" name='search_input'>
 <input type='submit' value='Search' formaction="koac_admindetails_res16.php">
 </form>
 </font>
</p>
...
$query->execute();
 if (!$query->rowCount() == 0) {
	...
	while ($results = $query->fetch()) {
		...
		echo "<td>".$results['Parent_or_Guardian']."</td>";
		echo "<td>".$results['StreetAddress']."</td>";
		echo "<td>".$results['Suburb']."</td>";
		echo "<td>".$results['PhoneNo']."</td>";
		echo "<td>".$results['Birthdate']."</td>";
