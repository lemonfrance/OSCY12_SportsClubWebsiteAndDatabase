...
<title> Application (Employment) - Admin </title>
...
<p class="titles" id="bodyttl"> Application - Employment Applications </p>
...
$employ_applicant = $_POST['employ_applicant'];

$sql = "SELECT * FROM club_employment WHERE ID LIKE '$employ_applicant'";
$result = mysqli_query($conn,$sql);
if ($result){
	while ($addemploy = mysqli_fetch_assoc($result){
		$fname = $addemploy['FirstName'];
		$lname = $addemploy['LastName'];
		$st = $addemploy['StreetAddress'];
		$sub = $addemploy['Suburb'];
		$phone = $addemploy['PhoneNo'];
		$birth = $addemploy['Birthdate'];
		$schl_wrk = $addemploy['School_Workplace'];
		$job = $addemploy['ApplyJob'];
	}
	$payment = 0;
	$haspaid = 'FALSE';
	if ($gr == 'Junior'){
	...
	$acc_sql = "INSERT INTO club_staff(FirstName, LastName, StreetAddress, Suburb, PhoneNo, Birthdate, School_Workplace, Position) VALUES('$fname', '$lname', '$st', '$sub', '$phone', '$birth', '$schl_wrk', '$job') ";
	$acc_result = mysqli_query($conn,$acc_sql);
	if($acc_result){
		echo "<p> Member number &nbsp".$employ_applicant."&nbsp has been accepted as a new member. </p>";
	}
	else{
...
<p>
<input type="submit" value="Go Back to Application (Employment) - Admin" formaction="koac_adminemploy16.php"/>
</p>
