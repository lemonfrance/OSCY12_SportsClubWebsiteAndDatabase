...
<title> Database (Points) - Admin </title>
...
body{
 ...
 <p class='titles' id='bodyttl'> Database - Points </p>

 <p>
 <font id='orangefont'>
 <form method='POST'>
 <input type='text' id="searchbox" name='search_input'>
 <input type='submit' value='Search' formaction="koac_adminpoints_res16.php">
 </font>
 </form>
</p>
<p>
...
$query = $pdo->prepare("select * from $members RIGHT JOIN $records ON $members.MemberNo = $records.MemberNo where $members.MemberNo LIKE '%$search%' OR FirstName LIKE '%$search%' OR LastName LIKE '%$search%'");
...
$query->execute();
         if (!$query->rowCount() == 0) {
            ...
            while ($results = $query->fetch()) {
				echo "<tr><td>";			
                echo $results['MemberNo'];
				echo "</td><td>";
                echo $results['FirstName'];
				echo "</td><td>";
				echo $results['LastName'];
				echo "</td><td>";
				echo $results['EventDate'];
				echo "</td><td>";
				echo $results['Points'];
				echo "</td></tr>";				
            }		
