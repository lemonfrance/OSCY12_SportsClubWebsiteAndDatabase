...
<?php
if (isset($_POST['modann_ttl'])){
	$modann_ttl = $_POST['modann_ttl'];
	echo "<p> Title has been modified.</p>";
}
if (empty($_POST['modann_ttl'])){
	echo "<p> Title not modified.</p>";
}

if (isset($_POST['modann_body'])){
	$modann_body = $_POST['modann_body'];
	echo "<p> Body has been modified.</p>";
}
if (empty($_POST['modann_body'])){
	echo "<p>Body not modified.</p>";
}

if (isset($_POST['modann'])){
	$modann = $_POST['modann'];
	echo "";
}
if (empty($_POST['modann'])){
	echo mysqli_errno($conn).":".mysqli_error($conn);
}

$sql = "UPDATE club_bulletin 
SET Title = '$modann_ttl', Body = '$modann_body'
WHERE BulletinSection LIKE 'Announcements' AND Title LIKE '$modann'";
mysqli_select_db($conn,$dbname);
$result = mysqli_query($conn,$sql);

if($result){
	echo "Successfully updated announcement data.";
}
else{
	echo mysqli_errno($conn).":".mysqli_error($conn);
}
