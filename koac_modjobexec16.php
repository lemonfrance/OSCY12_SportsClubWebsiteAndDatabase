...
<?php
if (isset($_POST['modjob_name'])){
	$modjob_name = $_POST['modjob_name'];
	echo "<p> Vacant job name has been modified.</p>";
}
if (empty($_POST['modjob_name'])){
	echo "<p> Vacant job name not modified.</p>";
}

if (isset($_POST['modjob_desc'])){
	$modjob_desc = $_POST['modjob_desc'];
	echo "<p> Job description has been modified.</p>";
}
if (empty($_POST['modjob_desc'])){
	echo "<p>Job description not modified.</p>";
}
...
if (empty($_POST['modjob'])){
	echo "<p> Error </p>";
	echo mysqli_errno($conn).":".mysqli_error($conn);
}

$sql = "UPDATE club_vacantjobs
SET Job = '$modjob_name', JobDescription = '$modjob_desc'
WHERE Job LIKE '$modjob'";

mysqli_select_db($conn,$dbname);
$result = mysqli_query($conn,$sql);

if($result){
	echo "Successfully updated vacant jobs data.";
}
else{
	echo mysqli_errno($conn).":".mysqli_error($conn);
}
